      ...
      <TopWrapper>
        ...
        <TitleWrapper>
          <TagWrapper>
            ...
            {themeInfo.genreList && themeInfo.genreList.map((genre, index) => <Tag key={index}>{genre}</Tag>)}
            <Tag>{themeInfo.playTime}분</Tag>
          </TagWrapper>
          <Title>{themeInfo.theme}</Title>
        </TitleWrapper>
        <ReservationButton onClick={() => handleThemeState(themeInfo.themeId)}>예약하기</ReservationButton>
      </TopWrapper>
      <Synopsis>{themeInfo.synopsis || '제공되는 줄거리가 없습니다.'}</Synopsis>
    </ComponentWrapper>
  );
}
